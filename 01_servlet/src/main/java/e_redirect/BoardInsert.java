...
public class BoardInsert extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("게시글 등록 요청용 Servlet 실행 (JDBC 과정을 통해 전달된 데이터 insert 진행)");
		
		// 요청 시 전달된 값 뽑아서 각 변수에 기록
		...
		// 성공적으로 추가되었다는 가정하에
		// 응답페이지 : 다시 게시글 전체 조회 페이지 (단, 새로 추가된 데이터도 조회되서 보여져야됨)
		
		
		//request.getRequestDispatcher("/e_redirect/boardList.html").forward(request, response);
		/*
		 * 잘못된 응답방법 : forward
		 * 문제점1. 포워딩을 통해 해당 페이지가 보여지긴 하나 여전히 /servlet/insert.bo 가 노출되
		 * 문제점2. 새로 추가된 데이터를 다시 조회해서 뿌려야만 하는데 새로이 조회가 진행되지 않음
		 * 
		 * 해결방법
		 * Insert 후에 다시 select하고 게시글 전체조회 페이지로 이동해야됨
		 * 근데 DB로 부터 데이터를 조회해서 해당 페이지로 이동하는 Servlet이 이미 존재함 (BoardList)
		 * => 해당 Servlet을 재 호출하면 됨 (URL 재요청, redirect)
		 */
		response.sendRedirect("/servlet/list.bo"); // => /list.bo 의 Servlet을 재실행
		
		/*
		 * *redirect
		 *  1. 기존에 정의해둔 Servlet을 재실행 시키는 개념 (URL 요청과 동일)
		 *  2. 등록 및 수정/삭제 후에 다시 조회 페이지를 요청할때 주로 사용함
		 *  3. redirect 시 현재 서블릿에 존재하는 request, response 전달하지 않음
		 *  4. 요청할 URL 작성시 contextPath 부터 작성
